...
def to_sft_samples(
    ...
    if annotator_type in ("classifier", "rule"):
        ...
        for row in _iter_done_llm_rows(store):
            ...
            # pet-schema v3.2.1 does not yet have an `images` field on
            # ShareGPTSFTSample (extra="forbid"). Use text-only format for now;
            # restore <image> + images field when schema is bumped to v3.3.0+.
